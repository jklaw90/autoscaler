...
var _ = utils.RecommenderE2eDescribe("VPA CRD object", func() {
	...
	f.It("generates recommendations for native sidecar containers", framework.WithFeatureGate(features.NativeSidecar), func() {
		...
		pod := podList.Items[0] //get the first pod of the deployment to just ensure

		// Verify native sidecar is present
		gomega.Expect(len(pod.Spec.InitContainers)).To(gomega.Equal(1))
		sidecarName := pod.Spec.InitContainers[0].Name
		gomega.Expect(sidecarName).To(gomega.Equal("native-sidecar"))
	...
	f.It("tracks resource usage for native sidecar containers independently", framework.WithFeatureGate(features.NativeSidecar), func() {
